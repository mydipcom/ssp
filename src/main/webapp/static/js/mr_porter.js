	...
	var handleTable = function() {
		...
		oTable = table
				.dataTable({
					...
					"serverSide" : true,
					"serverMethod" : "GET",
					"ajaxSource" : rootURI + "/product/mr_list?rand="
							+ Math.random(),
		...
		table.on('click', '#product_details', function() {
			...
			if(data.size_fit.length != 0){
				sizes = data.size_fit.split("#");
				size_str ='<ul>'
				$.each(sizes, function(i, value) {
					size_str +='<li>'+value+'</li>';
				});
				size_str +='</ul>';
				$('#size_fit').empty();
				$('#size_fit').append(size_str);
			}
			if(data.details_care.length != 0){
				details = data.details_care.split("#");
				detail_str ='<ul>'
			    $.each(details, function(i, value) {
			    	detail_str +='<li>'+value+'</li>';
				});
				detail_str +='</ul>';
				$('#details_care').empty();
				$('#details_care').append(detail_str);
			}
			$('#view').find('.product-page-options:eq(0)').find('span').text(
					data.pbrand);
			$('#view').find('.product-page-options:eq(1)').find(
					'.pull-left:eq(0)').find('span').text(data.prodid);
			$('#view').find('.product-page-options:eq(1)').find(
					'.pull-left:eq(1)').find('span').text(data.pcat);
